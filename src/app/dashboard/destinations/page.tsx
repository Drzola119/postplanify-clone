...
export default function DestinationsPage() {
  ...
  return (
    <div className="px-3 lg:px-6 pt-5 lg:pt-8 pb-3 lg:pb-6 max-w-4xl">
      <PageHeader
        title="Destinations"
        subtitle="Send publishing events to webhooks, Zapier, or your own endpoint whenever a post is published, fails, or is scheduled."
        cta={
          <button
            type="button"
...
function EmptyState({ onCreate }: { onCreate: () => void }) {
  return (
    <div className="rounded-xl border border-dashed border-zinc-200 bg-white py-12 px-6 text-center">
      ...
      <p className="mt-3 text-sm font-medium text-zinc-900">No destinations yet</p>
      <p className="mt-1 text-xs text-zinc-500 max-w-sm mx-auto">
        Add a webhook or Zapier hook to be notified when posts are published, scheduled, or fail.
      </p>
      <button
        type="button"
        onClick={onCreate}
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-5 relative">
        ...
        <h2 className="text-base font-semibold text-zinc-900">New destination</h2>
        <p className="mt-1 text-xs text-zinc-500">We'll send a signed POST to your URL when matching events occur.</p>
        ...
        <form onSubmit={submit} className="mt-4 space-y-4">
          ...
          <Field label="URL">
            <input
              ...
              placeholder="https://example.com/hooks/postplanify"
              className="w-full h-9 rounded-md border border-zinc-200 bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900/10"
            />
          </Field>
          ...
          <Field label="Signing secret (optional)">
            <input
              ...
              placeholder="min 8 chars; sent as X-PP-Signature"
              className="w-full h-9 rounded-md border border-zinc-200 bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900/10"
            />
          </Field>
