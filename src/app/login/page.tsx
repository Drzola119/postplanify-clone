...
function LoginForm() {
  ...
  async function exchangeSession() {
    try {
      ...
      if (!idToken) return; // No token yet, rely on client-side auth
      ...
      if (!res.ok) {
        // Server-side session failed — that's fine, client-side auth
        // handles protection via DashboardLayout's AuthContext guard.
        console.warn("Server session not available. Using client-side auth.");
      }
    } catch {
      // Network error or server down — proceed with client-side auth
  ...
  return (
    <CenteredCard>
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Sign in to your PostPlanify account.
      </p>
