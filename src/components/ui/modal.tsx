  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      ...
      <div
        ...
        aria-modal="true"
        className={cn(
          "relative w-full max-h-[calc(100vh-2rem)] flex flex-col rounded-xl bg-card text-card-foreground shadow-2xl animate-in fade-in-0 zoom-in-95 duration-200",
          sizes[size],
          className
        )}
      >
        ...
        {footer && <div className="px-5 py-3 border-t border-border flex items-center justify-end gap-2 bg-muted/50 rounded-b-xl shrink-0">{footer}</div>}
      </div>
    </div>
  );
}
