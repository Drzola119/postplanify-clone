...
if (!CDN && process.env.NODE_ENV !== "production") {
  // eslint-disable-next-line no-console -- dev-only warning for missing CDN env
  console.warn("[bunny] BUNNY_CDN_HOSTNAME not set — upload URLs will be empty until configured");
}
